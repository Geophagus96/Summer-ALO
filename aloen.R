...
alo_mis = foreach(i = 1:length(CV_mis$glmnet.fit$lambda), .combine = c) %do% {
  alo_pred = aloeldual(y_mis,X_mis,CV_mis$glmnet.fit$beta[, i], CV_mis$glmnet.fit$lambda[i], a)
  
  return(mean((alo_pred - y_mis)^2))
}
alo_hvy = foreach(i = 1:length(CV_hvy$glmnet.fit$lambda), .combine = c) %do% {
  alo_pred = aloeldual(y_hvy,X_hvy,CV_hvy$glmnet.fit$beta[, i], CV_hvy$glmnet.fit$lambda[i], a)
  
  return(mean((alo_pred - y_hvy)^2))
}
alo_cor = foreach(i = 1:length(CV_cor$glmnet.fit$lambda), .combine = c) %do% {
  alo_pred = aloeldual(y_cor,X_cor,CV_cor$glmnet.fit$beta[, i], CV_cor$glmnet.fit$lambda[i], a)
