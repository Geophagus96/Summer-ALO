...
vec poilassoALO(const vec &beta, const mat &X, const vec &y, const double &lambda){
  vec theta = y - exp(X*beta);
  double n = X.n_rows;
...
//[[Rcpp::export]]
vec poielnetALO(const vec &beta, const mat &X, const vec &y, const double &lambda, const double &alpha){
  double n = X.n_rows;
